...
import { of } from "rxjs";
import HttpUnauthorizedExceptionFilter from "../../src/filter/http-unauthorized-exception.filter";
...
describe("httpUnauthorizedExceptionFilter", () => {
  const filter: HttpUnauthorizedExceptionFilter = new HttpUnauthorizedExceptionFilter();

  it("should catch and log the error", () => {
    expect.assertions(7);
    const mockException: UnauthorizedException = new UnauthorizedException();

    mockException.name = "UnauthorizedException";
    jest
