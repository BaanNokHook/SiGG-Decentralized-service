...
describe("login", () => {
  ...
  it("renders correctly without JWT", () => {
    ...
    expect(container).toMatchInlineSnapshot(`
      <div>
        ...
        >
          This page acts as a demonstrator to simulate the Sample University - Bachelor's Programme issuing Diploma. During the login simulation, we will check that you have correctly followed the sigg onboarding process. If the verification fails, you will be redirected to the onboarding page.
        </p>
        <button
  ...
  it("renders correctly with empty JWT", () => {
    ...
    localStorage.setItem("Bachelor-JWT", "");
    const history = createMemoryHistory();
    const { container } = render(
      <Auth>
        <Router history={history}>
  ...
  it("renders correctly with malformed JWT", () => {
    ...
    localStorage.setItem("Bachelor-JWT", "123");
    const history = createMemoryHistory();
    const { container } = render(
      <Auth>
        <Router history={history}>
  ...
  it("should redirect the user to the wallet after requesting DID Auth", async () => {
    ...
    expect(localStorageSpy).toHaveBeenCalledWith("Bachelor-nonce", "123");
  ...
  it("should display an error when the DID Auth response JWT is not approved by the backend", async () => {
    ...
    window.localStorage.setItem("Bachelor-nonce", nonce);
  ...
  it("should display an error when the DID Auth response JWT is missing some props", async () => {
    ...
    const token = JWT.sign(payload, key, {
      // audience: "sigg-wallet", <- missing prop
      expiresIn: "2 hours",
      header: { typ: "JWT" },
      subject: "test",
    });
