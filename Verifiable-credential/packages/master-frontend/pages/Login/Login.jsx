...
function Login() {
  ...
  useEffect(() => {
    ...
    async function validateLogin() {
      try {
        ...
        const requestBody = {
          didAuthResponseJwt,
          nonce: window.localStorage.getItem("Master-nonce"),
        };

        const requestHeaders = new Headers();
        requestHeaders.append("Content-Type", "application/json");
        await fetch(
          `${REACT_APP_BACKEND_EXTERNAL_URL}/api/masters/did-auth-validations`,
          {
            method: "POST",
          ...
          if (mounted) {
            ...
            window.localStorage.setItem("Master-JWT", didAuthResponseJwt);
  ...
  async function handleLogin() {
    ...
    fetch(`${REACT_APP_BACKEND_EXTERNAL_URL}/api/masters/did-auth`, {
      method: "GET",
    })
      .then((response) => {
      ...
      .then((res) => {
        ...
        window.localStorage.setItem("Master-nonce", res.nonce);
  ...
  return (
    <>
      ...
      <P>
        ...
        Master&apos;s Programme issuing Diploma. During the login simulation, we
        will check that you have correctly followed the SiGG onboarding process.
        If the verification fails, you will be redirected to the onboarding
        page.
      </P>
      <button
        onClick={handleLogin}
