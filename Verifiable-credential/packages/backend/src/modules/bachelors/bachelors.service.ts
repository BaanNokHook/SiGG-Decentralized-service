import {
    ...
    } from "./bachelors.utils";
    
    @Injectable()
    export class BachelorsService {
      private readonly logger = new Logger(BachelorsService.name);
      ...
      async requestPresentation(body: RequestPresentationDto): Promise<any> {
        ...
        try {
          ...
          await this.walletService.requestPresentation({
            ...
            subscriberURL: `${this.configService.get(
              "internalUrl"
            )}/api/bachelors/presentations`,
            redirectURL,
            target,
          });
      ...
      async didAuth(): Promise<{ redirectUrl: string; nonce: string }> {
        ...
        return this.walletService.didAuth(
          accessToken,
          `${this.configService.get("publicUrl")}/bachelor/login`
        );
      }
